...
export class BrapiRepository extends IStockRepository {
  ...
  async search(query, type = "stock") {
    return withSpan("brapi.search", { "query": query, "type": type }, async (span) => {
      ...
      if (config.brapi.token)    params.append("token", config.brapi.token);
      ...
      try {
        ...
        // Filtra ativos inválidos, suspensos ou deslistados:
        // - sem ticker ou sem nome
        // - preço explicitamente zero (suspenso/em leilão)
        const results = allResults.filter(s => {
          if (!s.ticker || !s.name)                return false;
        ...
        logger.error("brapi.search failed", {
          "query":   query,
          "error":   err.message,
        });
        return [];
  ...
  async getQuote(tickers) {
    ...
    return withSpan("brapi.getQuote", { "tickers": tickerList.join(",") }, async (span) => {
      ...
      try {
        ...
        // Ticker não encontrado — brapi devolve 404 ou results vazio
        ...
        // Filtra ativos sem preço válido (suspensos, em leilão ou deslistados)
